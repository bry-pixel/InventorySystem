...
if (isset($_POST['cart_data'])) {
    ...
    if (!empty($cart_items)) {
        ...
        $create_txn_sql = "INSERT INTO transactions (txn_time, user_id, total) VALUES ('{$txn_time_esc}', " . 
                          ($user_id_val !== null ? "'{$user_id_val}'" : "NULL") . ", 0)";
        if (!$db->query($create_txn_sql)) {
            $session->msg('d', 'Failed to create transaction.');
            log_activity('Sale Failed', 'Failed to create transaction record', 'add_sale.php');
        ...
        foreach ($cart_items as $item) {
            ...
            if (!$product) {
                ...
                log_activity('Sale Error', "Product ID {$product_id} not found during sale", 'add_sale.php');
                continue;
            }
            ...
            if ($quantity > (float)$product['quantity']) {
                ...
                log_activity('Stock Error', "Not enough stock for {$product['name']}", 'add_sale.php');
                continue;
            }
            ...
            if ($db->query($sql)) {
                ...
                log_activity('Sale Error', "Failed to insert sale for {$product['name']}", 'add_sale.php');
            }
        }
        ...
        if ($success_count > 0) {
            ...
            redirect("reciept.php?id=" . (int)$txn_id, false);
        } else {
            // Cleanup empty transaction if nothing succeeded
            $db->query("DELETE FROM transactions WHERE id = '{$txn_id}'");
            $session->msg('d', "No sales were recorded. Transaction removed.");
            log_activity('Sale Failed', 'Transaction removed — no sales recorded', 'add_sale.php');
            redirect($_SERVER['PHP_SELF'], false);
        }
    } else {
...
<div class="row">
    <!-- Products -->
    <div class="col-md-8">
        <h4><i class="glyphicon glyphicon-barcode" style="color: white;"></i>
            <strong style="color:white;">Products</strong>
        </h4>
        ...
        <div class="cart-container">
            ...
            <form method="post">
                ...
                <div class="text-center">
                    <button type="submit" class="btn btn-success btn-m" style="border-radius:20px; padding:6px 15px; margin:5px;">
                        <i class="fa fa-check"></i> Complete Sale
                    </button>
                    <a href="sales.php" class="btn btn-outline-primary rounded-pill px-3 btn-m" style="border-radius:20px; padding:6px 15px; margin:5px;">
            ...
            <tr>
                ...
                <td>
                    <input type="number" min="1" max="${item.stock}" value="${item.qty}" 
                           ...
                           style="width:60px; text-align:center;"> ${item.unit}
                </td>
